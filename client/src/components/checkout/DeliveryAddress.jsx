...
const checkPincodeValidity = async (pincode) => {
  try {
    ...
    const response = await axios.get(
      `http://localhost:5000/api/pincodes/check/${pincode}`,
      {
        timeout: 3000,
        headers: {
  ...
  if (name === 'pincode' && value.length === 6) {
    ...
    try {
      ...
      const response = await axios.get(`http://localhost:5000/api/pincodes/check/${trimmedPincode}`, {
        timeout: 3000,
        headers: {
          'Content-Type': 'application/json'
        }
...
if (response.data.valid) {
    ...
    try {
      ...
      if (addressData._id) {
        response = await axios.put(
          `http://localhost:5000/api/address/${currentUser._id}/addresses/${addressData._id}`,
          addressData
        );
      } else {
  ...
  const handleRemoveAddress = async (addressId) => {
    try {
      await axios.delete(
        `http://localhost:5000/api/address/${currentUser._id}/addresses/${addressId}`
      );
